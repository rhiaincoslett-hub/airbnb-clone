...
/**
 * Update listing: GET by id to pre-fill, same form as create, PUT on submit.
 */
export function UpdateListingPage() {
    ...
    async function load() {
      try {
        ...
        setForm({
          ...
          guests: acc.guests ?? '',
          type: acc.type || 'Entire place',
          price: acc.price ?? '',
          amenities: Array.isArray(acc.amenities) ? acc.amenities : [],
          images: Array.isArray(acc.images) ? [...acc.images] : [],
          weeklyDiscount: acc.weeklyDiscount ?? '',
          cleaningFee: acc.cleaningFee ?? '',
          serviceFee: acc.serviceFee ?? '',
          occupancyTaxes: acc.occupancyTaxes ?? '',
        });
      } catch (err) {
        if (!cancelled) setServerError(err.message || 'Failed to load listing');
      } finally {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        setUploadError(data.message || (res.status === 401 ? 'Please log in again.' : `Upload failed (${res.status})`));
        return;
      }
      if (data.urls?.length) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setServerError(data.message || 'Failed to update listing');
        return;
      }
      navigate('/admin');
    } catch (err) {
  ...
  return (
    <div className={styles.form}>
      <h1 className={styles.title}>Update listing</h1>
      <form onSubmit={handleSubmit}>
        {serverError && <div className={styles.serverError} role="alert">{serverError}</div>}
        <Input label="Title" name="title" value={form.title} onChange={(e) => update('title', e.target.value)} error={errors.title} required />
        <Input label="Location" name="location" value={form.location} onChange={(e) => update('location', e.target.value)} error={errors.location} required />
        <Input label="Description" name="description" value={form.description} onChange={(e) => update('description', e.target.value)} />
        ...
        <div className={styles.row}>
          ...
          <div className={styles.field}>
            <Input label="Guests" name="guests" type="number" min="1" value={form.guests} onChange={(e) => update('guests', e.target.value)} />
          </div>
        </div>
        <div className={styles.row}>
          <div className={styles.field}>
            <Input label="Type" name="type" value={form.type} onChange={(e) => update('type', e.target.value)} />
          </div>
          <div className={styles.field}>
            <Input label="Price per night" name="price" type="number" min="0" step="0.01" value={form.price} onChange={(e) => update('price', e.target.value)} error={errors.price} required />
          </div>
        </div>
        <div className={styles.field} style={{ marginBottom: 'var(--spacing-md)' }}>
        ...
        <div className={styles.row}>
          <div className={styles.field}>
            <Input label="Weekly discount %" name="weeklyDiscount" type="number" min="0" max="100" value={form.weeklyDiscount} onChange={(e) => update('weeklyDiscount', e.target.value)} />
          </div>
          <div className={styles.field}>
            <Input label="Cleaning fee" name="cleaningFee" type="number" min="0" value={form.cleaningFee} onChange={(e) => update('cleaningFee', e.target.value)} />
          </div>
        </div>
        <div className={styles.row}>
          <div className={styles.field}>
            <Input label="Service fee" name="serviceFee" type="number" min="0" value={form.serviceFee} onChange={(e) => update('serviceFee', e.target.value)} />
          </div>
          <div className={styles.field}>
            <Input label="Occupancy taxes" name="occupancyTaxes" type="number" min="0" value={form.occupancyTaxes} onChange={(e) => update('occupancyTaxes', e.target.value)} />
          </div>
        </div>
        <Button type="submit" label={submitLoading ? 'Saving…' : 'Save changes'} disabled={submitLoading} />
      </form>
    </div>
  );
}
